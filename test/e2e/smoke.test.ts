...
export async function run(): Promise<void> {
  ...
  mocha.suite.addTest(
    new Mocha.Test(
      'extension activates and registers the sidebar webview view',
      async () => {
        ...
        const views: Array<{ id: string }> | undefined =
          ext.packageJSON?.contributes?.views?.claudeUsageMonitor;
        assert.ok(Array.isArray(views), 'packageJSON.contributes.views.claudeUsageMonitor is missing');
        assert.strictEqual(
          views[0]?.id,
          'claudeUsageMonitor.view',
          'First view id should be claudeUsageMonitor.view',
        );
      },
    ),
  );
  ...
  mocha.suite.addTest(
    new Mocha.Test(
      'status bar item is visible with non-empty text after watcher fires',
      async () => {
        ...
        assert.ok(
          typeof props?.['claudeUsageMonitor.warningTokens'] === 'object',
          'warningTokens configuration property must be declared',
        );
        assert.ok(
          typeof props?.['claudeUsageMonitor.criticalTokens'] === 'object',
          'criticalTokens configuration property must be declared',
        );
      },
    ),
  );
